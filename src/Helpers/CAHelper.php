...
namespace Kipl\Moduleadmin\Helpers;
...
class CAHelper
{
	// $names = CAHelper::generateModuleNames($module_name);
	...
	// CAHelper::parseValues($field['popup_vals']);
	...
	// CAHelper::log("info", "", $commandObject);
	...
	public static function recurse_copy($src,$dst) {
		...
		while(false !== ( $file = readdir($dir)) ) {
			if (( $file != '.' ) && ( $file != '..' )) {
				if ( is_dir($src . '/' . $file) ) {
					CAHelper::recurse_copy($src . '/' . $file,$dst . '/' . $file);
				}
				else {
					// ignore files
			...
			foreach ($objects as $object) {
				if ($object != "." && $object != "..") {
					if (is_dir($dir."/".$object))
						CAHelper::recurse_delete($dir."/".$object);
					else
						unlink($dir."/".$object);
				}
			}
	...
	// $password = CAHelper::gen_password();
	...
	// CAHelper::createThumbnail($filepath, $thumbpath, $thumbnail_width, $thumbnail_height);
	...
	// CAHelper::print_menu_editor($menu)
	public static function print_menu_editor($menu) {
		$editing = \Collective\Html\FormFacade::open(['route' => [config('moduleadmin.adminRoute').'.ca_menus.destroy', $menu->id], 'method' => 'delete', 'style'=>'display:inline']);
		...
		$childrens = \Kipl\Moduleadmin\Models\Menu::where("parent", $menu->id)->orderBy('hierarchy', 'asc')->get();

		if(count($childrens) > 0) {
			$str .= '<ol class="dd-list">';
			foreach($childrens as $children) {
				$str .= CAHelper::print_menu_editor($children);
			}
			$str .= '</ol>';
		}
		$str .= '</li>';
	...
	public static function print_menu($menu, $active = false) {
		...
		$str = '<li'.$treeview.' '.$active_str.'><a href="'.url(config("moduleadmin.adminRoute") . '/' . $menu->url ) .'"><i class="fa '.$menu->icon.'"></i> <span>'.CAHelper::real_module_name($menu->name).'</span> '.$subviewSign.'</a>';

		if(count($childrens)) {
			$str .= '<ul class="treeview-menu">';
			foreach($childrens as $children) {
				$str .= CAHelper::print_menu($children);
			}
			$str .= '</ul>';
		}
		$str .= '</li>';
	...
	// CAHelper::print_menu_topnav($menu)
	public static function print_menu_topnav($menu, $active = false) {
		...
		$str = '<li '.$treeview.''.$active_str.'><a '.$treeview2.' href="'.url(config("moduleadmin.adminRoute") . '/' . $menu->url ) .'">'.CAHelper::real_module_name($menu->name).$subviewSign.'</a>';

		if(count($childrens)) {
			$str .= '<ul class="dropdown-menu" role="menu">';
			foreach($childrens as $children) {
				$str .= CAHelper::print_menu_topnav($children);
			}
			$str .= '</ul>';
		}
		$str .= '</li>';
	...
	public static function setenv($param, $value) {
		...
		$envfile = CAHelper::openFile('.env');
		$line = CAHelper::getLineWithString('.env', $param.'=');
