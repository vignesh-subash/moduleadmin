...
class CAFormMaker
{
	...
	public static function input($module, $field_name, $default_val = null, $required2 = null, $class = 'form-control', $params = [])
	{
		...
		if(Module::hasFieldAccess($module->id, $module->fields[$field_name]['id'], $access_type = "write")) {
			...
			if($unique && !isset($params['unique'])) {
				...
				$params['adminRoute'] = config('moduleadmin.adminRoute');
				if(isset($row)) {
					$params['isEdit'] = true;
	...
	**/
	...
	/**
	* Display field using blade directive @ca_display
	**/
					...
					if(starts_with($fieldObj['popup_vals'], "@")) {
						if($value != 0) {
							...
							if(isset($moduleVal->id)) {
								$value = "<a href='".url(config("moduleadmin.adminRoute")."/".$moduleVal->name_db."/".$value)."' class='label label-primary'>".$values[$value]."</a> ";
							} else {
						...
						if(starts_with($fieldObj['popup_vals'], "@")) {
							...
							foreach ($values as $key => $val) {
								if(in_array($key, $valueSel)) {
									$valueOut .= "<a href='".url(config("moduleadmin.adminRoute")."/".$moduleVal->name_db."/".$key)."' class='label label-primary'>".$val."</a> ";
								}
							}
						} else {
							$valueSel = json_decode($value);
	...
	**/
	public static function ca_access($module_id, $access_type = "view", $user_id = 0)
	{
	...
	**/
	public static function ca_field_access($module_id, $field_id, $access_type = "view", $user_id = 0)
	{
