...
class CAInstall extends Command
{
	/**
	 ...
	 * @var string
	 */
	protected $description = 'Install ModuleAdmin Package. Generate whole structure for /admin.';
	...
	{
		try {
			$this->info('ModuleAdmin installation started...');

			$from = base_path('vendor/kipl/moduleadmin/src/Installs');
			$to = base_path();
			...
			if ($this->confirm("Want to set your Database config in the .env file ?", true)) {
				...
				if(CAHelper::laravel_ver() == 5.5) {
					...
					CAHelper::setenv("DB_HOST", $db_data['host']);
					CAHelper::setenv("DB_PORT", $db_data['port']);
				}
				...
				CAHelper::setenv("DB_DATABASE", $db_data['db']);
				CAHelper::setenv("DB_USERNAME", $db_data['dbuser']);
				CAHelper::setenv("DB_PASSWORD", $db_data['dbpass']);
			}

			if(env('CACHE_DRIVER') != "array") {
				config(['cache.default' => 'array']);
				CAHelper::setenv("CACHE_DRIVER", "array");
			}
			...
			if ($this->confirm("This process may change/append to the following of your existing project files:"
				...
				$this->replaceFolder($from."/app/Controllers/CA", $to."/app/Http/Controllers/CA");
				if(CAHelper::laravel_ver() == 5.5) {
				...
				if ($this->confirm('Would you like to customize this url ?', false)) {
					...
					$laconfigfile =  $this->openFile($to."/config/moduleadmin.php");
					$arline = CAHelper::getLineWithString($to."/config/moduleadmin.php", "'adminRoute' => 'admin',");
					$laconfigfile = str_replace($arline, "    'adminRoute' => '" . $custom_admin_route . "',", $laconfigfile);
					file_put_contents($to."/config/moduleadmin.php", $laconfigfile);
					config(['moduleadmin.adminRoute' => $custom_admin_route]);
				}
				*/
				...
				// la-assets
				$this->line('Generating ModuleAdmin Public Assets...');
				...
				// $this->call('db:seed', ['--class' => 'ModuleAdminSeeder']);
				...
				if(CAHelper::getLineWithString('config/database.php', "dump_command_path") == -1) {
					...
					$mysqldriverline = CAHelper::getLineWithString('config/database.php', "'driver' => 'mysql'");
				...
				//if(!$this->fileContains($to."/app/Http/routes.php", "moduleadmin.adminRoute")) {
				if(CAHelper::laravel_ver() == 5.5) {
					if(CAHelper::getLineWithString($to."/routes/web.php", "require __DIR__.'/admin_routes.php';") == -1) {
						$this->appendFile($from."/app/routes.php", $to."/routes/web.php");
					}
					$this->copyFile($from."/app/admin_routes.php", $to."/routes/admin_routes.php");
				} else {
					if(CAHelper::getLineWithString($to."/app/Http/routes.php", "require __DIR__.'/admin_routes.php';") == -1) {
				...
				$this->info("\nModuleAdmin successfully installed.");
				$this->info("You can now login from yourdomain.com/".config('moduleadmin.adminRoute')." !!!\n");
